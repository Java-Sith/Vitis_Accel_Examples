...
static constexpr char kBitstream[] = 
    "/home/ubuntu/docs/lleon/bitstreams/ad08-vivado/ad08.bit";
...
int main(int argc, char** argv) {
  ...
  std::shared_ptr<IAccelerator> elemwise = platform->GetAccelerator(kElemWiseAddr);
  // Get a data mover
  ...
  std::shared_ptr<IMemory> buf_mem_a = mover->GetBuffer(
      size_a * sizeof(uint16_t), matmul->GetMemoryBank(0));
  std::shared_ptr<IMemory> buf_mem_b = mover->GetBuffer(
      size_b * sizeof(uint16_t), matmul->GetMemoryBank(1));
  std::shared_ptr<IMemory> buf_mem_c = mover->GetBuffer(
      size_c * sizeof(uint16_t), matmul->GetMemoryBank(2));
  std::shared_ptr<IMemory> buf_mem_d = mover->GetBuffer(
      size_c * sizeof(uint16_t), elemwise->GetMemoryBank(1));
  std::shared_ptr<IMemory> buf_mem_e = mover->GetBuffer(
      size_c * sizeof(uint16_t), elemwise->GetMemoryBank(2));
  ...
  uint16_t* bo_d_map = buf_mem_d->HostAddress<uint16_t>().get();
  uint16_t* bo_e_map = buf_mem_e->HostAddress<uint16_t>().get();

  // Filling data
  std::cout << "Filling Buffers\n";
  DataT as = 0.002, bs = 0.003, ds = 0.5;
  std::cout << "A: " << std::endl;
  ...
  std::fill(bo_d_map, bo_d_map + size_c, ds.V);
  std::fill(bo_e_map, bo_e_map + size_c, 0);
  ...
  matmul->Attach(XMATMUL_CONTROL_ADDR_A_DATA, buf_mem_a);
  matmul->Attach(XMATMUL_CONTROL_ADDR_B_DATA, buf_mem_b);
  matmul->Attach(XMATMUL_CONTROL_ADDR_C_DATA, buf_mem_c);

  elemwise->Write(XELEMENTWISE_CONTROL_ADDR_SIZE_DATA, &size_c, 1);
  elemwise->Write(XELEMENTWISE_CONTROL_ADDR_OP_DATA, &op, 1);
  elemwise->Attach(XELEMENTWISE_CONTROL_ADDR_IN1_DATA, buf_mem_c);
  elemwise->Attach(XELEMENTWISE_CONTROL_ADDR_IN2_DATA, buf_mem_d);
  elemwise->Attach(XELEMENTWISE_CONTROL_ADDR_OUT_R_DATA, buf_mem_e);
  ...
  buf_mem_d->Sync(SyncType::HostToDevice);
  ...
  std::cout << "INFO: Trigger Download" << std::endl;
  buf_mem_e->Sync(SyncType::DeviceToHost);
